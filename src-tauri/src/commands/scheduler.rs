...
pub struct AutoEmailResult { pub success: bool, pub message: String, pub rows_sent: usize, pub timestamp: String }

struct TaggedRow { client: String, account: String, fy: String, month: String, statement_path: String, status: String, count: usize }
...
fn aggregate_tagged(rows: &[TaggedRow]) -> Vec<TaggedRow> {
    ...
    for r in rows {
        ...
        let key = format!("{}||{}||{}||{}||{}", r.client, r.account, r.fy, r.month, parent);
        let entry = map.entry(key).or_insert_with(|| TaggedRow {
            client: r.client.clone(), account: r.account.clone(), fy: r.fy.clone(), month: r.month.clone(),
...
fn generate_summary_excel(rows: &[TaggedRow], temp_dir: &PathBuf) -> Result<String, String> {
    ...
    let hf = Format::new().set_bold().set_font_size(11.0).set_font_color(Color::White).set_background_color(Color::RGB(0x615FFF));
    let ok = Format::new().set_font_color(Color::RGB(0x1D9E75));
    let ef = Format::new().set_font_color(Color::RGB(0xE25C5C));
    let headers = ["Client", "Account", "FY", "Month", "Statement Type", "File Count"];
    let widths = [18, 18, 14, 14, 25, 12];
    for (col, (h, w)) in headers.iter().zip(widths.iter()).enumerate() {
        sheet.set_column_width(col as u16, *w).map_err(|e| e.to_string())?;
        sheet.write_string_with_format(0, col as u16, *h, &hf).map_err(|e| e.to_string())?;
    }
    for (i, row) in rows.iter().enumerate() {
        ...
        let fmt = if row.count > 0 { &ok } else { &ef };
        sheet.write_number_with_format(r, 5, row.count as f64, fmt).map_err(|e| e.to_string())?;
    }
    let ts = chrono::Local::now().format("%Y%m%d_%H%M%S").to_string();
    let fp = temp_dir.join(format!("Kredo_AutoReport_{}.xlsx", ts));
    workbook.save(fp.to_str().unwrap_or("report.xlsx")).map_err(|e| e.to_string())?;
...
fn generate_summary_html(rows: &[TaggedRow], total: usize, filled: usize, empty: usize, completion: f64) -> String {
    ...
    struct ChipEntry { client: String, account: String, fy: String, stype: String, is_fy: bool, months: std::collections::BTreeMap<String, usize>, total: usize }
    let mut chip_map: std::collections::BTreeMap<String, ChipEntry> = std::collections::BTreeMap::new();

    for r in rows {
        let is_fy = r.month == dash;
        let key = format!("{}||{}||{}||{}||{}", r.client, r.account, r.fy, r.statement_path, if is_fy { "fy" } else { "m" });
        let entry = chip_map.entry(key).or_insert_with(|| ChipEntry {
            client: r.client.clone(), account: r.account.clone(), fy: r.fy.clone(), stype: r.statement_path.clone(),
    ...
    for entry in chip_map.values() {
        let cur = format!("{}||{}||{}", entry.client, entry.account, entry.fy);
        ...
        let chips_html = if entry.is_fy {
            format!("<span style=\"display:inline-block;padding:3px 8px;border-radius:3px;font-size:8px;font-weight:600;background:{};color:{};\">FY {}</span>",
                ...
                if entry.total > 0 { "#1D9E75" } else { "#E25C5C" }, entry.total)
        } else {
        ...
        </tr>", td_s, entry.client, td_s, entry.account, td_s, fy_short, td_s, type_name, td_s, chips_html));
    }
...
<td style=\"padding-left:10px;vertical-align:middle;\"><div style=\"color:white;font-size:14px;font-weight:600;\">Kredo File Manager</div><div style=\"color:rgba(255,255,255,0.65);font-size:10px;margin-top:2px;\">Automated status report</div></td>
</tr></table></td></tr>
...
<tr><td width=\"50%\" style=\"text-align:center;padding:12px 0;border-right:1px solid #F0EFF5;border-bottom:1px solid #F0EFF5;\"><div style=\"font-size:22px;font-weight:700;color:#131126;\">{}</div><div style=\"font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;color:#908EAF;margin-top:3px;\">Total</div></td>
<td width=\"50%\" style=\"text-align:center;padding:12px 0;border-bottom:1px solid #F0EFF5;\"><div style=\"font-size:22px;font-weight:700;color:#1D9E75;\">{}</div><div style=\"font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;color:#908EAF;margin-top:3px;\">Filled</div></td></tr>
<tr><td width=\"50%\" style=\"text-align:center;padding:12px 0;border-right:1px solid #F0EFF5;\"><div style=\"font-size:22px;font-weight:700;color:#E25C5C;\">{}</div><div style=\"font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;color:#908EAF;margin-top:3px;\">Empty</div></td>
<td width=\"50%\" style=\"text-align:center;padding:12px 0;\"><div style=\"font-size:22px;font-weight:700;color:#615FFF;\">{:.1}%</div><div style=\"font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;color:#908EAF;margin-top:3px;\">Complete</div></td></tr>
</table></td></tr>
...
<tr style=\"background:#F5F4FC;height:48px;\"><th style=\"{}\">Client</th><th style=\"{}\">Account</th><th style=\"{}\">FY</th><th style=\"{}\">Type</th><th style=\"{}padding:0 2px;\">Months</th></tr>
{}
</table></td></tr>
<tr><td style=\"padding:12px 16px;border-top:1px solid #F0EFF5;text-align:center;\"><div style=\"font-size:10px;color:#908EAF;\">Detailed audit attached as Excel</div><div style=\"font-size:9px;color:#C4C3D4;margin-top:4px;\">Automated report by Kredo File Manager</div></td></tr>
...
pub fn fy_month_keys_rust(fy: &str) -> Vec<String> {
    ...
    let end_year: u32 = if end_str.len() == 2 { format!("{}{}", &re_match[0][..2], end_str).parse().unwrap_or(0) } else { end_str.parse().unwrap_or(0) };
    let names = ["Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","Jan","Feb","Mar"];
    let years = [start_year,start_year,start_year,start_year,start_year,start_year,start_year,start_year,start_year,end_year,end_year,end_year];
    let indices = ["01","02","03","04","05","06","07","08","09","10","11","12"];
    indices.iter().zip(names.iter()).zip(years.iter()).map(|((i,n),y)| format!("{} {} {}", i, n, y)).collect()
}

#[tauri::command]
pub fn send_auto_email(
    app_handle: tauri::AppHandle, root_path: String, smtp_config: SmtpConfig,
    to: Vec<String>, cc: Vec<String>, subject: String,
    ...
    for entry in fs::read_dir(&root).map_err(|e| format!("{}", e))?.flatten() {
        if entry.path().is_dir() { let n = entry.file_name().to_string_lossy().to_string(); if !n.starts_with('.') && !n.starts_with('_') { entities.push(n); } }
    }
    entities.sort();

    // Walk Client → Account → FY
    for entity in &entities {
        let entity_dir = root.join(entity);
        let accounts = match fs::read_dir(&entity_dir) { Ok(e) => e, Err(_) => continue };
        for account_entry in accounts.flatten() {
            if !account_entry.path().is_dir() { continue; }
            let aname = account_entry.file_name().to_string_lossy().to_string();
            if aname.starts_with('.') || aname.starts_with('_') { continue; }
            let fys_iter = match fs::read_dir(account_entry.path()) { Ok(e) => e, Err(_) => continue };
            let mut fy_names: Vec<String> = fys_iter.flatten()
                .filter(|e| e.path().is_dir() && e.file_name().to_string_lossy().starts_with("FY"))
                .map(|e| e.file_name().to_string_lossy().to_string()).collect();
            fy_names.sort();

            for fy_name in &fy_names {
                match scan_audit(root_path.clone(), vec![entity.clone()], vec![aname.clone()], vec![fy_name.clone()]) {
                    Ok(result) => {
                        for r in result.rows {
                            tagged_rows.push(TaggedRow {
                                client: r.client, account: r.account, fy: r.fy,
                                month: r.month, statement_path: r.statement_path, status: r.status, count: r.count,
                            });
                        }
                    }
                    Err(_) => continue,
    ...
    let payload = EmailPayload { to, cc, subject, body: html, attachments: vec![excel_path] };
    let result = send_email(smtp_config, payload)?;
    let timestamp = chrono::Local::now().to_rfc3339();
    Ok(AutoEmailResult { success: result.success, message: format!("Auto report sent: {} rows across {} clients", total, entities.len()), rows_sent: total, timestamp })
}
